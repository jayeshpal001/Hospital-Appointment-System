...
import { 
  ...
  FaCalendarCheck, FaPhoneAlt, FaEdit, FaSignOutAlt, FaGraduationCap, 
        ...
        >
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex flex-row md:flex-col gap-3">
                    <button className="p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/30 transition-all text-white group" title="Edit Profile">
                        <FaEdit className="text-xl group-hover:text-cyan-400" />
                    </button>
                    <button className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 hover:border-red-500/40 transition-all text-red-400 group" title="Logout">
            ...
            <motion.div variants={itemVariants} className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 hover:border-white/20 transition-all group">
                    ...
                    <p className="text-3xl font-bold text-white ml-2">
                        ...
                        <span className="text-sm text-gray-500 font-normal ml-2">/ visit</span>
                    </p>
                </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-between border-b border-white/5 pb-3">
                             <div className="flex flex-col">
                                 <span className="text-gray-500 text-xs flex items-center gap-1 mb-1"><FaBirthdayCake /> Age</span>
                                 <span className="text-white font-medium">{age} Years</span>
                             </div>
                             <div className="flex flex-col text-right">
                                 <span className="text-gray-500 text-xs flex items-center gap-1 justify-end mb-1"><FaVenusMars /> Gender</span>
                        ...
                        <div className="flex justify-between items-center text-sm border-b border-white/5 pb-3">
                            <span className="text-gray-500 flex items-center gap-2"><FaGlobe /> Nationality</span>
                        ...
                        <div className="flex justify-between items-center text-sm border-b border-white/5 pb-3">
                            ...
                            <span className="text-white truncate max-w-37.5" title={user.email}>{user.email}</span>
                        </div>

                        {/* Address */}
                        <div className="pt-1">
                            <span className="text-gray-500 text-xs flex items-center gap-1 mb-2"><FaMapMarkerAlt /> Clinic Address</span>
            ...
            <motion.div variants={itemVariants} className="lg:col-span-2">
                <div className="h-full bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 relative overflow-hidden">
                    ...
                    <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
                        <FaCalendarCheck className="text-cyan-400" /> Availability Schedule
                    </h2>

                    {/* Days Visualizer */}
                    <div className="mb-10">
                        <label className="text-sm text-gray-400 mb-4 block font-medium">Working Days</label>
                    ...
                    {/* Time Slots Grid */}
                    <div>
                        <label className="text-sm text-gray-400 mb-4 block font-medium">Daily Slots</label>
                        {availableSlots.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {availableSlots.map((slot, index) => (
                                    <div key={index} className="flex items-center gap-4 bg-black/30 border border-white/10 p-4 rounded-xl hover:border-cyan-500/30 transition-colors group">
                                        ...
                                        <div>
                                            <p className="text-white font-bold tracking-wide text-lg">
                                                {slot.startTime} <span className="text-gray-500 text-sm font-normal">to</span> {slot.endTime}
                                            </p>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                                                <p className="text-xs text-green-400 uppercase tracking-wider font-bold">Active Slot</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
